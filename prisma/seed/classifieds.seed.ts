...
import { BodyType, ClassifiedStatus, Colour, CurrencyCode, FuelType, OdoUnit, Prisma, PrismaClient, Transmission, ULEZCompliance } from "@prisma/client";
...
export async function seedClassified(prisma: PrismaClient) {
    ...
    for (let i = 0; i < 25; i++){
        ...
        if (make.models.length === 0) continue;
        ...
        const year = faker.date.between({
            from: new Date(1925, 0, 1), to: new Date()
        }).getFullYear();
        ...
        classifiedsData.push({
            ...
            odoReading: faker.number.int({ min: 0, max: 2000000 }),
